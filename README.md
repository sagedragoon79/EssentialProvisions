...
Sixteen features across four categories, every one OFF by default. Open the in-game settings panel, pick what you want, ignore the rest.
...
A few features are original EP designs built directly on FF's own internal systems: Consumable Control and Surplus Selling drive FF's trading-post quota system from observed consumption telemetry, and Short Walks leverages FF's already-existing (but underused) Hungarian assignment infrastructure to expand its scope to all villagers, not just new arrivals.

## Build

```
dotnet build src/EssentialProvisions.csproj -p:Configuration=Release -p:Platform=x64 -p:GameDir="G:\SteamLibrary\steamapps\common\Farthest Frontier"
```
