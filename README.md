...
Nineteen features across four categories, every one OFF by default. Open the in-game settings panel, pick what you want, ignore the rest.
...
- **Learned Hands** — Educated villagers work faster at every job. FF's own guide says educated workers are more efficient, but the game never actually implements it — education only gates a handful of jobs. This adds the missing work-rate bonus: each educated villager gets a multiplicative speed-up (+10% per education level) on everything they do — resource gathering, field work, **and crafting/manufacturing**, from chopping wood to baking bread. Per-worker, so a building's output scales with its workers' education, and it applies retroactively to villagers already educated. On/off — the rate is fixed by design.
- **Workplace Mastery** — Villagers get faster the longer they stay in a job. Each worker earns a small, growing work-rate bonus per in-game year of tenure (**+1%/year, capped at +25%** at 25 years), applied to gathering, field work, **and crafting/manufacturing**, and it stacks on top of Learned Hands. Tenure is tracked per occupation — switching jobs banks the old job's experience and starts a fresh clock — and persists across saves. Hover a villager's profession icon in the worker-assignment picker to see their tenure. On/off — the rate is fixed by design. Existing villagers start fresh, since the save has no job-start date to seed from.
- **Penny Pincher** — Auto-stand-down rat catchers when no infestations are active, and guard towers in peacetime. Re-enables automatically when threats reappear, with a 2-day cooldown after raids end to avoid flickering. Won't touch a building you currently have selected. A **"Stand Down Guard Towers" sub-toggle** lets you keep guard towers entirely under your own control while still auto-managing rat catchers — switching it off immediately re-mans any towers Penny Pincher had stood down (and leaves ones you disabled yourself alone).
- **Idle Hands** — A small icon appears on production buildings whose workers spend too much time idle. Tunable threshold.
- **Rabies: Treatable (longshot)** — Vanilla rabies is a guaranteed death sentence no healer can ever cure. This opens a slim, healer-dependent chance: a rabid villager falls bedridden and is treated in a healer's sick bed, where the cure odds scale with care — roughly ~9% at a staffed Healer's House up to ~18% at a stocked Hospital. The cost: while ill they stop working and occupy a sick bed for up to 60 days, and with no reachable healer bed they're bedridden at home and almost certainly still die. A deliberate balance change, OFF by default; only affects villagers infected after you enable it.
